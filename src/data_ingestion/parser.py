...
- job_family: use this priority order strictly:
    1. Clear title keyword → use it directly. "Data Engineer" → data_engineering. "ML Engineer" → ml_engineering. "Analytics Engineer" → data_analytics. "Data Analyst" → data_analytics. "Research Scientist" → research. "Engineering Manager" → management. Any role with "Analyst" in the title → data_analytics (e.g. "Business Analyst", "Financial Analyst", "Operations Analyst", "Marketing Analyst", "Reporting Analyst"), unless the title also contains a more specific family keyword that takes precedence (e.g. "Data Science Analyst" → data_science, "ML Analyst" → ml_engineering). Do not second-guess a clear title keyword. Exception: "Data Scientist" is treated as ambiguous regardless of the keyword — proceed to step 2 and decide from primary responsibilities.
...
- skills_required: HARD BOUNDARY — this field is for technical and domain knowledge only that might appear anywhere in the posting. A skill named in a responsibility bullet is implicitly required. Any interpersonal, behavioural, or soft skill — regardless of where it appears in the posting — belongs in skills_soft, never here. If in doubt about whether something is a soft skill, it goes to skills_soft. Examples that must NOT appear here: communication, leadership as an interpersonal trait (e.g. "strong leadership skills", "natural leader", "leading through influence", "leading cross-functional collaboration") — note: leadership belongs in skills_required only when it specifies what is being led in technical or domain terms (e.g. "experience leading data engineering teams", "technical leadership of ML projects", "leading ML model deployments"); facilitation, ideation, interpersonal skills, collaboration, creativity, problem-solving (as a behavioural trait), stakeholder management, and similar tokens are soft skills and do not belong here.
  ...
  CRITICAL: scan the entire posting, including the responsibilities section. The same sentence may contribute to two fields simultaneously — this is correct and expected. Responsibility bullets frequently embed skill tokens; extract those tokens into skills_required as well as into key_responsibilities. For example (one of many possible cases), "Optimise jobs to utilise Kafka, Hadoop, Spark Streaming and Kubernetes resources" should produce a key_responsibilities entry AND add each named technology to skills_required. This applies equally to any skill — tool, language, methodology, domain area — that appears anywhere in the responsibilities section.
